...
$admin_id = $_SESSION['admin_id'];
    ...
    if(isset($_POST['add_product'])){
        ...
        if(mysqli_num_rows($select_product_name) > 0){
            ...
            if($image_size > 2000000){
                ...
                $select_product_query = mysqli_query($conn, "INSERT INTO `products`(name, price, image) VALUES
                ('$name', '$price', '$image')") or die('query failed');
    ...
    if(isset($_POST['update_product'])){
        ...
        $select_update = mysqli_query($conn, "UPDATE `products` SET name = '$update_name', price = '$update_price' WHERE id = '$update_p_id' ") or die('query failed');
        ...
        if(!empty($update_image)){
            if($update_image_size > 2000000){
                $message[] = 'update image size is too large!';
            }else{
...
<section class="add-product">
    ...
    <form action="" method="post" enctype="multipart/form-data">
        ...
        <input type="price" name="price" class="box" placeholder="enter product price" required>
...
<?php
   ...
   <?php
          ...
          echo '<script>document.querySelector(".edit-container").style.display="none";</script>';
      }
   ?>
